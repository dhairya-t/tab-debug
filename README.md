...
We reproduced an existing bug in [Transform](https://github.com/ritz078/transform): use **Load File → Fetch URL**, click Fetch URL for one file, then submit a second URL while the first download is pending, and the slower first download can overwrite the newer selection. Both requests return 200, but the editor shows the wrong file. The public walkthrough illustrates this with staging and production JSON in a working JSON-to-YAML view. Those sample names describe a plausible user mistake, not a reported user incident. The walkthrough advances only when you click: submit staging, submit production, deliver production, then deliver staging. It holds the real response bodies before the loader updates the editor. HTTP timing remains untouched in the tools, and app state also lists the order of editor updates. You can operate the URL loader yourself without pauses, then read the actual WebMCP state and response history.
