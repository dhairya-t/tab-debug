...
const sensitive =
  /password|secret|token|authorization|cookie|email|phone|api.?key/i;
export function sanitize(
  ...
  if (Array.isArray(value))
    return value.slice(0, 24).map((v) => sanitize(v, depth + 1, seen));
  const out: Record<string, unknown> = Object.create(null);
  for (const [key, item] of Object.entries(value).slice(0, 24))
    out[key.slice(0, 80)] = sensitive.test(key)
      ? '[redacted]'
...
function bounded(value: unknown): unknown {
  ...
  return JSON.stringify(clean).length > 8000
    ? { truncated: true, reason: '8KB record budget' }
    : clean;
}
function pathOnly(url: string): string {
  try {
    return new URL(url, 'http://pagescope.invalid').pathname.slice(0, 240);
  } catch {
    return '[invalid URL]';
  }
}
export class PageScope {
  ...
  constructor(options: { capacity?: number } = {}) {
    ...
    if (!Number.isInteger(capacity) || capacity < 4 || capacity > 1000)
      throw new Error('Capacity must be an integer from 4 to 1000');
  ...
  record(kind: EventKind, label: string, data: unknown = {}) {
    const event: TraceEvent = {
      ...
      time: Date.now(),
      kind,
      label: String(sanitize(label)).slice(0, 160),
      data: bounded(data),
    };
  ...
  setState(name: string, state: unknown) {
    if (!name || name.length > 80)
      throw new Error('State source name must be 1 to 80 characters');
    if (
      !Object.hasOwn(this.#state, name) &&
      Object.keys(this.#state).length >= 24
    )
      throw new Error('Maximum 24 state sources per page');
    const clean = sensitive.test(name) ? '[redacted]' : bounded(state);
    if (
      Object.hasOwn(this.#state, name) &&
      JSON.stringify(this.#state[name]) === JSON.stringify(clean)
    )
      return;
  ...
  async fetch(
    ...
    try {
      ...
      if (generation === this.#generation) {
        this.record('response', `${response.status} ${path}`, {
          requestId,
      ...
      if (generation === this.#generation) {
        this.record('response', `Network failure: ${path}`, {
          requestId,
          status: 0,
          ok: false,
  ...
  callTool(name: string, input: unknown = {}) {
    if (!toolNames.includes(name as ToolName))
      throw new Error(`Unknown PageScope tool: ${name}`);
    if (!input || typeof input !== 'object' || Array.isArray(input))
      throw new Error('Input must be an object');
    const values = input as Record<string, unknown>;
    if (Object.keys(values).some((key) => key !== 'limit'))
    ...
    if (name === 'get_page_context')
      return {
        ...
        stateSources: Object.keys(snapshot.state),
        latestError:
          snapshot.events.filter((e) => e.kind === 'error').at(-1) ?? null,
        guarantees: [
          'page-local',
          'bounded records',
          'opt-in state',
    ...
    const events = snapshot.events.filter(
      (e) =>
        ...
        (name === 'inspect_errors'
          ? e.kind === 'error'
          : e.kind === 'request' || e.kind === 'response'),
    );
    return {
      ...base,
      total: events.length,
