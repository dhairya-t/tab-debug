...
const context = await browser.newContext({
  ...
  recordVideo:
    mode === 'instrumented'
      ? {
          dir: 'test-results/transform-video',
...
try {
  ...
  const evidence = {
    ...
    revision: 'ff7557939be351706f4dc6f71cc375e3bc64c225',
    mode,
    method:
      'Real upstream app and file-loading UI. Only two JSON responses and their completion order are controlled by Playwright.',
    requestOrder: ['first.json', 'second.json'],
    responseOrder: ['second.json', 'first.json'],
    chosenFileVisible,
    finalVisibleEditor: await editor.innerText(),
    expectedFile: 'second.json',
    actualFile: mode === 'fixed' ? 'second.json' : 'first.json',
  };
  if (mode !== 'original') {
    evidence.webmcp = {};
    ...
    assert.match(
      evidence.webmcp.inspect_state.state.Editor1.value,
      mode === 'fixed' ? /second.json/ : /first.json/,
    );
    assert.equal(
      evidence.webmcp.inspect_requests.events.filter(
  ...
  writeFileSync(
    `docs/verification/transform-${mode}.json`,
    JSON.stringify(evidence, null, 2) + '\n',
  );
  console.log(
    `${mode}: last selected second.json; visible ${evidence.actualFile}${evidence.webmcp ? '; native WebMCP verified' : ''}`,
  );
